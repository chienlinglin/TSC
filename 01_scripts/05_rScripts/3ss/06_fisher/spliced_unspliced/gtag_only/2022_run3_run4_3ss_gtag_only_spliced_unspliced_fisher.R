...
for (i in 1:nrow(gtable)){
  ...
  ssi <- t1[c(wtname,mtname),c(15,13)]
  ssi[is.na(ssi)] <- 0
...
for (i in 1:nrow(gtable)){
  wtname<-as.character(gtable[i,2])
  mtname<-as.character(gtable[i,3])
  wtsum <- sum(t1[wtname, c(15,13)])
  wt_spliced <- t1[wtname, 15]
  wt_unspliced <- t1[wtname, 13]
  mtsum <- sum(t1[mtname, c(15,13)])
  mt_spliced <- t1[mtname, 15]
...
setwd("~/Desktop/phD/Rotation/2/TSC alignment/re_aligment/20220318_modified_with with --mp10:4 -k 1 noncaonal_ss 1000/3ss/fisher/spliced_unspliced/gtag_only/")
write.csv(ptable, file="2022_run3_run4_3_1_gtag_only_spliced_unspliced_chisq_fisher.csv")

##draw histogram of pvalues and fvalues
png(file="hist_3_1_gtag_only_pval_fval_su.png")
par(mfrow=c(1,2))
...
for (i in 1:nrow(gtable)){
  wtname <- as.character(gtable[i,2])
  mtname <- as.character(gtable[i,3])
  ssi <- t2[c(wtname,mtname),c(15,13)]
  ssi[is.na(ssi)] <- 0
  if (sum(ssi[mtname, c(1,2)]) == 0) {pval<-NA}
  else if (sum(ssi[wtname, c(1,2)]) == 0) {pval<-NA}
  else{pval<-chisq.test(ssi)$p.value}
...
for (i in 1:nrow(gtable)){
  wtname<-as.character(gtable[i,2])
  mtname<-as.character(gtable[i,3])
  wtsum <- sum(t2[wtname, c(15,13)])
  wt_spliced <- t2[wtname, 15]
  wt_unspliced <- t2[wtname, 13]
  mtsum <- sum(t2[mtname, c(15,13)])
  mt_spliced <- t2[mtname, 15]
...
setwd("~/Desktop/phD/Rotation/2/TSC alignment/re_aligment/20220318_modified_with with --mp10:4 -k 1 noncaonal_ss 1000/3ss/fisher/spliced_unspliced/gtag_only/")
write.csv(ptable, file="2022_run3_run4_3_2_gtag_only_spliced_unspliced_chisq_fisher.csv")

##draw histogram of pvalues and fvalues
png(file="hist_3_2_gtag_only_pval_fval_su.png")
par(mfrow=c(1,2))
hist(ptable$pval,breaks = 20)
hist(ptable$fval,breaks = 20)
dev.off()
...
for (i in 1:nrow(gtable)){
  wtname <- as.character(gtable[i,2])
  mtname <- as.character(gtable[i,3])
  ssi <- t3[c(wtname,mtname),c(15,13)]
  ssi[is.na(ssi)] <- 0
  if (sum(ssi[mtname, c(1,2)]) == 0) {pval<-NA}
  else if (sum(ssi[wtname, c(1,2)]) == 0) {pval<-NA}
  else{pval<-chisq.test(ssi)$p.value}
...
for (i in 1:nrow(gtable)){
  wtname<-as.character(gtable[i,2])
  mtname<-as.character(gtable[i,3])
  wtsum <- sum(t3[wtname, c(15,13)])
  wt_spliced <- t3[wtname, 15]
  wt_unspliced <- t3[wtname, 13]
  mtsum <- sum(t3[mtname, c(15,13)])
  mt_spliced <- t3[mtname, 15]
...
setwd("~/Desktop/phD/Rotation/2/TSC alignment/re_aligment/20220318_modified_with with --mp10:4 -k 1 noncaonal_ss 1000/3ss/fisher/spliced_unspliced/gtag_only/")
write.csv(ptable, file="2022_run3_run4_3_3_gtag_only_spliced_unspliced_chisq_fisher.csv")

##draw histogram of pvalues and fvalues
png(file="hist_3_3_gtag_only_pval_fval_su.png")
par(mfrow=c(1,2))
hist(ptable$pval,breaks = 20)
hist(ptable$fval,breaks = 20)
dev.off()
...
setwd("~/Desktop/phD/Rotation/2/TSC alignment/re_aligment/20220318_modified_with with --mp10:4 -k 1 noncaonal_ss 1000/3ss/fisher/spliced_unspliced/gtag_only/")
f1 <- read.csv(file = "2022_run3_run4_3_1_gtag_only_spliced_unspliced_chisq_fisher.csv",row.names = 1)
f2 <- read.csv(file = "2022_run3_run4_3_2_gtag_only_spliced_unspliced_chisq_fisher.csv",row.names = 1)
f3 <- read.csv(file = "2022_run3_run4_3_3_gtag_only_spliced_unspliced_chisq_fisher.csv",row.names = 1)
...
write.csv(ft_sort_f, file="2022_run3_run4_3ss_gtag_only_spliced_unspliced_sort_fvals.csv")
