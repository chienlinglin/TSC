...
set <- su_f %>% filter(wt_sum_3 == 3) %>% filter(mt_sum_3 == 3) %>% select(mt) %>% unlist() #1350
set1_ <- su_f %>% filter(qval_3 == 3) %>% filter(se_difference_3 == 3) %>% select(mt) %>% unlist() #76
set1 <- intersect(set1_,set) #spliced_unspliced = 42
set2_ <- cn_f %>% filter(qval_3 == 3) %>% filter(se_difference_3 == 3) %>% select(mt) %>% unlist() #263
set2 <- intersect(set2_,set) #canonical_nocanonical = 144
set3_ <- cnu_f %>% filter(qval_3 == 3) %>% filter(se_difference_3 == 3) %>% select(mt) %>% unlist() #283
set3 <- intersect(set3_,set) #cn+uns = 145
...
pdf(file="2025_3ss_significant_gtag_only_30_fdr0.05_100reads_venn_diagramm.pdf")
