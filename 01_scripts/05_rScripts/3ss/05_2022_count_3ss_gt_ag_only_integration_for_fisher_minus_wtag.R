...
View(gtable)
gtable$"wt-mt" <- paste0("wt-",gtable$V2,"mt-",gtable$V3)
...
exon2 <- read.csv(file = "~/Desktop/phD/Rotation/2/TSC alignment/re_aligment/20220318_modified_with with --mp10:4 -k 1 noncaonal_ss 1000/5ss/fisher/hi_3ss_5ss_exon_fixed2_final_add_GT_AG.csv",row.names = 1)
intron2 <- read.csv(file = "~/Desktop/phD/Rotation/2/TSC alignment/re_aligment/20220318_modified_with with --mp10:4 -k 1 noncaonal_ss 1000/5ss/fisher/hi_3ss_5ss_intron_fixed2_final_add_GT_AG.csv",row.names = 1)


ex_wt <- exon2[,c(4,50,51,52,53,66,67,68)]
in_wt <- intron2[,c(103,93,94,83,84,101,102,42)]
colnames(ex_wt) <- c("name","ag1_start","ag1_end","ag2_start","ag2_end","can_intron_start","can_intron_end","3/5ss")
colnames(in_wt) <- c("name","ag1_start","ag1_end","ag2_start","ag2_end","can_intron_start","can_intron_end","3/5ss")
wt <- rbind(ex_wt,in_wt)
wt_ag <- wt[which(str_detect(wt$`3/5ss`,pattern = "3")),c(1:7)]

wt_ag$name2 <- wt_ag$name
wt_ag <- separate(wt_ag,"name2", into=c("name2","r"),sep="_chr")
wt_ag <- wt_ag[which(!is.na(wt_ag$r)),]

wt_ag$name2 <- paste0(wt_ag$name2,"_")
wt_ag <- wt_ag[!is.na(wt_ag$ag1_start),]
wt_ag$ag1 <- paste0(wt_ag$name2,wt_ag$can_intron_start,wt_ag$ag1_end)
wt_ag$ag2[!is.na(wt_ag$ag2_start)] <- paste0(wt_ag$name2[!is.na(wt_ag$ag2_start)],
                                             wt_ag$can_intron_start[!is.na(wt_ag$ag2_start)],
                                             wt_ag$ag2_end[!is.na(wt_ag$ag2_start)])
wt_ag <- wt_ag[,c(1,8,10,11)]


## hi: only intronic library has barcode
...
s1_wt1 <- merge(s1ag,wt_ag,by.x = "ag1",by.y = "ag1")     # 227
s1_wt2 <- merge(s1ag,wt_ag,by.x = "ag1",by.y = "ag2")     # 1
s1_barcode <- merge(s1ag,barcode_ag,by.x = "ag1",by.y = "ag") # 0
s1_barcode2 <- merge(s1ag,barcode_ag,by.x = "ag1",by.y = "ag2") # 0
s1_wt1_f <- s1_wt1[,c(1,2,9,10)]
s1_barcode_f <- s1_barcode[,c(1,2,9)]
colnames(s1_wt1_f) <- c("wt1_-ag_spliced","chrom","wt1_-ag_num","mt_name")
colnames(s1_barcode_f) <- c("barcode_+ag_spliced","chrom","barcode_+ag_num")
s1_wt1_f$"wt-mt" <- paste0("wt-",s1_wt1_f$chrom,"mt-",s1_wt1_f$mt_name)
...
s2_wt1 <- merge(s2ag,wt_ag,by.x = "ag2",by.y = "ag1")     # 209
s2_wt2 <- merge(s2ag,wt_ag,by.x = "ag2",by.y = "ag2")     # 1
s2_barcode <- merge(s2ag,barcode_ag,by.x = "ag2",by.y = "ag") # 0
s2_barcode2 <- merge(s2ag,barcode_ag,by.x = "ag2",by.y = "ag2") # 0
s2_wt1_f <- s2_wt1[,c(1,2,9,10)]
s2_barcode_f <- s2_barcode[,c(1,2,9)]
colnames(s2_wt1_f) <- c("wt1_-ag_spliced","chrom","wt1_-ag_num","mt_name")
colnames(s2_barcode_f) <- c("barcode_+ag_spliced","chrom","barcode_+ag_num")
s2_wt1_f$"wt-mt" <- paste0("wt-",s2_wt1_f$chrom,"mt-",s2_wt1_f$mt_name)
...
s3_wt1 <- merge(s3ag,wt_ag,by.x = "ag3",by.y = "ag1")     # 223
s3_wt2 <- merge(s3ag,wt_ag,by.x = "ag3",by.y = "ag2")     # 1
s3_barcode <- merge(s3ag,barcode_ag,by.x = "ag3",by.y = "ag") # 0
s3_barcode2 <- merge(s3ag,barcode_ag,by.x = "ag3",by.y = "ag2") # 0
s3_wt1_f <- s3_wt1[,c(1,2,9,10)]
s3_barcode_f <- s3_barcode[,c(1,2,9)]
colnames(s3_wt1_f) <- c("wt1_-ag_spliced","chrom","wt1_-ag_num","mt_name")
colnames(s3_barcode_f) <- c("barcode_+ag_spliced","chrom","barcode_+ag_num")
s3_wt1_f$"wt-mt" <- paste0("wt-",s3_wt1_f$chrom,"mt-",s3_wt1_f$mt_name)
...
no_s1_wt1 <- merge(no_s1,wt_ag,by.x = "ag",by.y = "ag1")     # 0
no_s1_wt2 <- merge(no_s1,wt_ag,by.x = "ag",by.y = "ag2")     # 0
...
no_s2_wt1 <- merge(no_s2,wt_ag,by.x = "ag",by.y = "ag1")     #0
no_s2_wt2 <- merge(no_s2,wt_ag,by.x = "ag",by.y = "ag2")     #0
...
no_s3_wt1 <- merge(no_s3,wt_ag,by.x = "ag",by.y = "ag1")     #0
no_s3_wt2 <- merge(no_s3,wt_ag,by.x = "ag",by.y = "ag2")     #0
...
# combine add ag file
s1_correct_2 <- merge(s1_correct_2,gtable[,-1],by.x="chrom",by.y="V2",all=TRUE)
s2_correct_2 <- merge(s2_correct_2,gtable[,-1],by.x="chrom",by.y="V2",all=TRUE)
s3_correct_2 <- merge(s3_correct_2,gtable[,-1],by.x="chrom",by.y="V2",all=TRUE)

s1_correct_2 <- merge(s1_correct_2,s1_wt1_f,by.x = "wt-mt",by.y = "wt-mt",all.x = TRUE)
s2_correct_2 <- merge(s2_correct_2,s2_wt1_f,by.x = "wt-mt",by.y = "wt-mt",all.x = TRUE)
s3_correct_2 <- merge(s3_correct_2,s3_wt1_f,by.x = "wt-mt",by.y = "wt-mt",all.x = TRUE)

s1_correct_2$"wt1_-ag_form" <- NA
s2_correct_2$"wt1_-ag_form" <- NA
s3_correct_2$"wt1_-ag_form" <- NA


s1_correct_2 <- merge(s1_correct_2,s1_barcode_f,by.x = "chrom.x",by.y = "chrom",all.x = TRUE)
s2_correct_2 <- merge(s2_correct_2,s2_barcode_f,by.x = "chrom.x",by.y = "chrom",all.x = TRUE)
s3_correct_2 <- merge(s3_correct_2,s3_barcode_f,by.x = "chrom.x",by.y = "chrom",all.x = TRUE)
...
for (i in 1:nrow(s1_correct_2)){
  if (is.na(s1_correct_2$`wt1_-ag_spliced`[i])) {a <- NA; b <- NA;  c <- NA; d <- NA}
  else if (s1_correct_2$`wt1_-ag_spliced`[i] == s1_correct_2$canonical[which(str_detect(s1_correct_2$chrom.x[i],
                                                                                        pattern = s1_correct_2$chrom.y[i]))]){a <- i}
  else if (s1_correct_2$`wt1_-ag_spliced`[i] != s1_correct_2$canonical[which(str_detect(s1_correct_2$chrom.x[i],
                                                                                        pattern = s1_correct_2$chrom.y[i]))]){b <- i}
  s1_correct_2$"wt1_-ag_form"[a] <- "can"
  s1_correct_2$"wt1_-ag_form"[b] <- "noncan"
}
...
for (i in 1:nrow(s1_correct_2)){
  if (is.na(s1_correct_2$`wt1_-ag_form`[i])) {a <- s1_correct_2$sum_spliced[i]; b <- s1_correct_2$num_canonical[i];
  c <- s1_correct_2$noncanonical[i]; d <- s1_correct_2$non_un[i]}
  else if (s1_correct_2$`wt1_-ag_form`[i] == "can"){b <- s1_correct_2$num_canonical[i]-s1_correct_2$`wt1_-ag_num`[i];c <- s1_correct_2$noncanonical[i];
  a <- b+c;  d <- c+s1_correct_2$s1_unspliced[i]}
  else if (s1_correct_2$`wt1_-ag_form`[i] == "noncan"){b <- s1_correct_2$num_canonical[i]; c <- s1_correct_2$noncanonical[i]-s1_correct_2$`wt1_-ag_num`[i]; 
  a <- b+c; d <- c+s1_correct_2$s1_unspliced[i]}
  s1_correct_2$"wt1_-ag_spliced_num"[i] <- a
  s1_correct_2$"wt1_-ag_can"[i] <- b
  s1_correct_2$"wt1_-ag_noncan"[i] <- c
  s1_correct_2$"wt1_-ag_non_un"[i] <- d
}
...
for (i in 1:nrow(s2_correct_2)){
  if (is.na(s2_correct_2$`wt1_-ag_spliced`[i])) {a <- NA; b <- NA;  c <- NA; d <- NA}
  else if (s2_correct_2$`wt1_-ag_spliced`[i] == s2_correct_2$canonical[which(str_detect(s2_correct_2$chrom.x[i],
                                                                                        pattern = s2_correct_2$chrom.y[i]))]){a <- i}
  else if (s2_correct_2$`wt1_-ag_spliced`[i] != s2_correct_2$canonical[which(str_detect(s2_correct_2$chrom.x[i],
                                                                                        pattern = s2_correct_2$chrom.y[i]))]){b <- i}
  s2_correct_2$"wt1_-ag_form"[a] <- "can"
  s2_correct_2$"wt1_-ag_form"[b] <- "noncan"
}
...
for (i in 1:nrow(s2_correct_2)){
  if (is.na(s2_correct_2$`wt1_-ag_form`[i])) {a <- s2_correct_2$sum_spliced[i]; b <- s2_correct_2$num_canonical[i];
  c <- s2_correct_2$noncanonical[i]; d <- s2_correct_2$non_un[i]}
  else if (s2_correct_2$`wt1_-ag_form`[i] == "can"){ b <- s2_correct_2$num_canonical[i]-s2_correct_2$`wt1_-ag_num`[i];
  c <- s2_correct_2$noncanonical[i];a <- b+c; d <- c+s2_correct_2$s2_unspliced[i]}
  else if (s2_correct_2$`wt1_-ag_form`[i] == "noncan"){ b <- s2_correct_2$num_canonical[i];
  c <- s2_correct_2$noncanonical[i]-s2_correct_2$`wt1_-ag_num`[i];a <- b+c; d <- c+s2_correct_2$s2_unspliced[i]}
  s2_correct_2$"wt1_-ag_spliced_num"[i] <- a
  s2_correct_2$"wt1_-ag_can"[i] <- b
  s2_correct_2$"wt1_-ag_noncan"[i] <- c
  s2_correct_2$"wt1_-ag_non_un"[i] <- d
}
...
for (i in 1:nrow(s3_correct_2)){
  if (is.na(s3_correct_2$`wt1_-ag_spliced`[i])) {a <- NA; b <- NA;  c <- NA; d <- NA}
  else if (s3_correct_2$`wt1_-ag_spliced`[i] == s3_correct_2$canonical[which(str_detect(s3_correct_2$chrom.x[i],
                                                                                        pattern = s3_correct_2$chrom.y[i]))]){a <- i}
  else if (s3_correct_2$`wt1_-ag_spliced`[i] != s3_correct_2$canonical[which(str_detect(s3_correct_2$chrom.x[i],
                                                                                        pattern = s3_correct_2$chrom.y[i]))]){b <- i}
  s3_correct_2$"wt1_-ag_form"[a] <- "can"
  s3_correct_2$"wt1_-ag_form"[b] <- "noncan"
}
...
for (i in 1:nrow(s3_correct_2)){
  if (is.na(s3_correct_2$`wt1_-ag_form`[i])) {a <- s3_correct_2$sum_spliced[i]; b <- s3_correct_2$num_canonical[i];
  c <- s3_correct_2$noncanonical[i]; d <- s3_correct_2$non_un[i]}
  else if (s3_correct_2$`wt1_-ag_form`[i] == "can"){b <- s3_correct_2$num_canonical[i]-s3_correct_2$`wt1_-ag_num`[i];
  c <- s3_correct_2$noncanonical[i]; a <- b+c; d <- c+s3_correct_2$s3_unspliced[i]}
  else if (s3_correct_2$`wt1_-ag_form`[i] == "noncan"){b <- s3_correct_2$num_canonical[i];
  c <- s3_correct_2$noncanonical[i]-s3_correct_2$`wt1_-ag_num`[i];a <- b+c;  d <- c+s3_correct_2$s3_unspliced[i]}
  s3_correct_2$"wt1_-ag_spliced_num"[i] <- a
  s3_correct_2$"wt1_-ag_can"[i] <- b
  s3_correct_2$"wt1_-ag_noncan"[i] <- c
  s3_correct_2$"wt1_-ag_non_un"[i] <- d
}
...
s1_correct_2$`wt-mt`[which(is.na(s1_correct_2$`wt-mt`))] <- s1_correct_2$chrom.x[which(is.na(s1_correct_2$`wt-mt`))]
s2_correct_2$`wt-mt`[which(is.na(s2_correct_2$`wt-mt`))] <- s2_correct_2$chrom.x[which(is.na(s2_correct_2$`wt-mt`))]
s3_correct_2$`wt-mt`[which(is.na(s3_correct_2$`wt-mt`))] <- s3_correct_2$chrom.x[which(is.na(s3_correct_2$`wt-mt`))]
...
write.csv(s1_correct_2,file = "3_1_correct_can_non_gtag_only_wt_minus_ag.csv",row.names = s1_correct_2$`wt-mt`)
write.csv(s2_correct_2,file = "3_2_correct_can_non_gtag_only_wt_minus_ag.csv",row.names = s2_correct_2$`wt-mt`)
write.csv(s3_correct_2,file = "3_3_correct_can_non_gtag_only_wt_minus_ag.csv",row.names = s3_correct_2$`wt-mt`)
...
dat_intron_mt <- intron_size1[,c(3,8,19)]     ## chrom, sum, canonical
...
# combine add ag file
can_1_N_filtered <- merge(can_1_N_filtered,gtable[,-1],by.x="chrom",by.y="V2",all=TRUE)
can_1_N_filtered <- merge(can_1_N_filtered,s1_wt1_f,by.x = "wt-mt",by.y = "wt-mt",all.x = TRUE)

can_1_N_filtered$"wt1_-ag_form" <- NA
can_1_N_filtered <- merge(can_1_N_filtered,s1_barcode_f,by.x = "chrom.x",by.y = "chrom",all.x = TRUE)
can_1_N_filtered$"barcode_+ag_form" <- NA


# add ag s1
can_1_N_filtered$"wt1_-ag_form"[which(can_1_N_filtered$`wt1_-ag_spliced` == can_1_N_filtered$most)] <- "can"
can_1_N_filtered$"wt1_-ag_form"[which(can_1_N_filtered$`wt1_-ag_spliced` != can_1_N_filtered$most)] <- "noncan"
...
for (i in 1:nrow(can_1_N_filtered)){
  if (is.na(can_1_N_filtered$`wt1_-ag_form`[i])) {a <- can_1_N_filtered$sum_spliced[i]; b <- can_1_N_filtered$num_canonical[i];
  c <- can_1_N_filtered$noncanonical[i]; d <- can_1_N_filtered$non_un[i]}
  else if (can_1_N_filtered$`wt1_-ag_form`[i] == "can"){b <- can_1_N_filtered$num_canonical[i]-can_1_N_filtered$`wt1_-ag_num`[i];
  c <- can_1_N_filtered$noncanonical[i]; a <- b+c; d <- c+can_1_N_filtered$s1_unspliced[i]}
  else if (can_1_N_filtered$`wt1_-ag_form`[i] == "noncan"){ b <- can_1_N_filtered$num_canonical[i];
  c <- can_1_N_filtered$noncanonical[i]-can_1_N_filtered$`wt1_-ag_num`[i]; a <- b+c;d <- c+can_1_N_filtered$s1_unspliced[i]}
  can_1_N_filtered$"wt1_-ag_spliced_num"[i] <- a
  can_1_N_filtered$"wt1_-ag_can"[i] <- b
  can_1_N_filtered$"wt1_-ag_noncan"[i] <- c
  can_1_N_filtered$"wt1_-ag_non_un"[i] <- d
}
...
#write csv correct can_non
setwd("~/Desktop/phD/Rotation/2/TSC alignment/re_aligment/20220318_modified_with with --mp10:4 -k 1 noncaonal_ss 1000/3ss/fisher/")
can_1_N_filtered$`wt-mt`[which(is.na(can_1_N_filtered$`wt-mt`))] <- can_1_N_filtered$chrom.x[which(is.na(can_1_N_filtered$`wt-mt`))]
#row.names(can_1_N_filtered) <- can_1_N_filtered$`wt-mt`
write.csv(can_1_N_filtered,file = "3_1_most_can_non_gtag_only_wt_minus_ag.csv",row.names = can_1_N_filtered$"wt-mt")
...
# combine add ag file
can_2_N_filtered <- merge(can_2_N_filtered,gtable[,-1],by.x="chrom",by.y="V2",all=TRUE)
can_2_N_filtered <- merge(can_2_N_filtered,s2_wt1_f,by.x = "wt-mt",by.y = "wt-mt",all.x = TRUE)

can_2_N_filtered$"wt1_-ag_form" <- NA
can_2_N_filtered <- merge(can_2_N_filtered,s2_barcode_f,by.x = "chrom.x",by.y = "chrom",all.x = TRUE)
can_2_N_filtered$"barcode_+ag_form" <- NA


# add ag s2
can_2_N_filtered$"wt1_-ag_form"[which(can_2_N_filtered$`wt1_-ag_spliced` == can_2_N_filtered$most)] <- "can"
can_2_N_filtered$"wt1_-ag_form"[which(can_2_N_filtered$`wt1_-ag_spliced` != can_2_N_filtered$most)] <- "noncan"
...
for (i in 1:nrow(can_2_N_filtered)){
  if (is.na(can_2_N_filtered$`wt1_-ag_form`[i])) {a <- can_2_N_filtered$sum_spliced[i]; b <- can_2_N_filtered$num_canonical[i];
  c <- can_2_N_filtered$noncanonical[i]; d <- can_2_N_filtered$non_un[i]}
  else if (can_2_N_filtered$`wt1_-ag_form`[i] == "can"){b <- can_2_N_filtered$num_canonical[i]-can_2_N_filtered$`wt1_-ag_num`[i];
  c <- can_2_N_filtered$noncanonical[i]; a <- b+c; d <- c+can_2_N_filtered$s2_unspliced[i]}
  else if (can_2_N_filtered$`wt1_-ag_form`[i] == "noncan"){ b <- can_2_N_filtered$num_canonical[i];
  c <- can_2_N_filtered$noncanonical[i]-can_2_N_filtered$`wt1_-ag_num`[i]; a <- b+c;d <- c+can_2_N_filtered$s2_unspliced[i]}
  can_2_N_filtered$"wt1_-ag_spliced_num"[i] <- a
  can_2_N_filtered$"wt1_-ag_can"[i] <- b
  can_2_N_filtered$"wt1_-ag_noncan"[i] <- c
  can_2_N_filtered$"wt1_-ag_non_un"[i] <- d
}
...
#write csv correct can_non
setwd("~/Desktop/phD/Rotation/2/TSC alignment/re_aligment/20220318_modified_with with --mp10:4 -k 1 noncaonal_ss 1000/3ss/fisher/")
can_2_N_filtered$`wt-mt`[which(is.na(can_2_N_filtered$`wt-mt`))] <- can_2_N_filtered$chrom.x[which(is.na(can_2_N_filtered$`wt-mt`))]
write.csv(can_2_N_filtered,file = "3_2_most_can_non_gtag_only_wt_minus_ag.csv",row.names = can_2_N_filtered$"wt-mt")
...
# combine add ag file
can_3_N_filtered <- merge(can_3_N_filtered,gtable[,-1],by.x="chrom",by.y="V2",all=TRUE)
can_3_N_filtered <- merge(can_3_N_filtered,s3_wt1_f,by.x = "wt-mt",by.y = "wt-mt",all.x = TRUE)

can_3_N_filtered$"wt1_-ag_form" <- NA
can_3_N_filtered <- merge(can_3_N_filtered,s3_barcode_f,by.x = "chrom.x",by.y = "chrom",all.x = TRUE)
can_3_N_filtered$"barcode_+ag_form" <- NA


# add ag s3
can_3_N_filtered$"wt1_-ag_form"[which(can_3_N_filtered$`wt1_-ag_spliced` == can_3_N_filtered$most)] <- "can"
can_3_N_filtered$"wt1_-ag_form"[which(can_3_N_filtered$`wt1_-ag_spliced` != can_3_N_filtered$most)] <- "noncan"
...
for (i in 1:nrow(can_3_N_filtered)){
  if (is.na(can_3_N_filtered$`wt1_-ag_form`[i])) {a <- can_3_N_filtered$sum_spliced[i]; b <- can_3_N_filtered$num_canonical[i];
  c <- can_3_N_filtered$noncanonical[i]; d <- can_3_N_filtered$non_un[i]}
  else if (can_3_N_filtered$`wt1_-ag_form`[i] == "can"){b <- can_3_N_filtered$num_canonical[i]-can_3_N_filtered$`wt1_-ag_num`[i];
  c <- can_3_N_filtered$noncanonical[i]; a <- b+c; d <- c+can_3_N_filtered$s3_unspliced[i]}
  else if (can_3_N_filtered$`wt1_-ag_form`[i] == "noncan"){ b <- can_3_N_filtered$num_canonical[i];
  c <- can_3_N_filtered$noncanonical[i]-can_3_N_filtered$`wt1_-ag_num`[i]; a <- b+c;d <- c+can_3_N_filtered$s3_unspliced[i]}
  can_3_N_filtered$"wt1_-ag_spliced_num"[i] <- a
  can_3_N_filtered$"wt1_-ag_can"[i] <- b
  can_3_N_filtered$"wt1_-ag_noncan"[i] <- c
  can_3_N_filtered$"wt1_-ag_non_un"[i] <- d
}
...
#write csv correct can_non
setwd("~/Desktop/phD/Rotation/2/TSC alignment/re_aligment/20220318_modified_with with --mp10:4 -k 1 noncaonal_ss 1000/3ss/fisher/")
can_3_N_filtered$`wt-mt`[which(is.na(can_3_N_filtered$`wt-mt`))] <- can_3_N_filtered$chrom.x[which(is.na(can_3_N_filtered$`wt-mt`))]
write.csv(can_3_N_filtered,file = "3_3_most_can_non_gtag_only_wt_minus_ag.csv",row.names = can_3_N_filtered$"wt-mt")



###########################################################################################################
############## find splice site near barcode ##############################################################
barcode <- intron[,c(96,69,70,42)]
barcode$"barcode_near" <- NA
barcode$"barcode_near"[which(barcode$X3..5.ss=="3ss")] <- barcode$barcode_position_end[which(barcode$X3..5.ss=="3ss")]
barcode$"barcode_near"[which(barcode$X3..5.ss=="5ss")] <- barcode$barcode_position_start[which(barcode$X3..5.ss=="5ss")]-5

barcode_s1 <- merge(s1,barcode[,c(1,5)],by.x = "chrom",by.y = "new_name")
barcode_s2 <- merge(s2,barcode[,c(1,5)],by.x = "chrom",by.y = "new_name")
barcode_s3 <- merge(s3,barcode[,c(1,5)],by.x = "chrom",by.y = "new_name")

barcode_s1_f <- barcode_s1[which((barcode_s1$end %in% c(barcode_s1$barcode_near:barcode_s1$barcode_near+5))==TRUE),]
barcode_s2_f <- barcode_s2[which((barcode_s2$end %in% c(barcode_s2$barcode_near:barcode_s2$barcode_near+5))==TRUE),]
barcode_s3_f <- barcode_s3[which((barcode_s3$end %in% c(barcode_s3$barcode_near:barcode_s3$barcode_near+5))==TRUE),]


#setwd("~/Documents/yiting/Hung Lun/home2_cllin_20191223/aligncomrev_0707/splice_near_barcode/")
#write.csv(barcode_s1_f,file = "s1_splice_near_5_barcode.csv")
#write.csv(barcode_s2_f,file = "s2_splice_near_5_barcode.csv")
#write.csv(barcode_s3_f,file = "s3_splice_near_5_barcode.csv")
