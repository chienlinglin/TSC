...
se_difference <- abs(wt_s_e$wt_splicing_efficiency-mt_s_e$mt_splicing_efficiency)
se_difference1 <- abs(wt_s_e$wt_splicing_efficiency1-mt_s_e$mt_splicing_efficiency1)
se_difference2 <- abs(wt_s_e$wt_splicing_efficiency2-mt_s_e$mt_splicing_efficiency2)
se_difference_1 <- cbind(se_difference,se_difference1,se_difference2,mt_s_e)
se_difference_1$se_difference_3 <- (se_difference_1$se_difference>=30) + (se_difference_1$se_difference1>=30) + (se_difference_1$se_difference2>=30)
...
set1_ <- su_f %>% filter(qval_3 == 3) %>% filter(se_difference_3 == 3) %>% select(mt) %>% unlist()
set1 <- intersect(set1_,set)
set2_ <- cn_f %>% filter(qval_3 == 3) %>% filter(se_difference_3 == 3) %>% select(mt) %>% unlist()
set2 <- intersect(set2_,set)
set3_ <- cnu_f %>% filter(qval_3 == 3) %>% filter(se_difference_3 == 3) %>% select(mt) %>% unlist()
...
gtable3$spliced_unspliced_fdr0.05_reads100_30 <- NA
gtable3$spliced_unspliced_fdr0.05_reads100_30[which(gtable3$mt %in% set1)] <- "su"
gtable3$canonical_noncan_fdr0.05_reads100_30 <- NA
gtable3$canonical_noncan_fdr0.05_reads100_30[which(gtable3$mt %in% set2)] <- "cn"
gtable3$canonical_noncan_un_fdr0.05_reads100_30 <- NA
gtable3$canonical_noncan_un_fdr0.05_reads100_30[which(gtable3$mt %in% set3)] <- "cnu"
...
most <- na.omit(intersect(intersect(s1$most,s2$most),s3$most))  # 2345
...
colnames(gtable3)[12:15] <- c("wt_can_intron_start","wt_can_intron_end","wt_nchar","wt_fasta")

gtable3 <- merge(gtable3,final3[,c(2,5,6,7,8)],by.x="mt",by.y="name",all.x=TRUE)
colnames(gtable3)[16:19] <- c("mt_can_intron_start","mt_can_intron_end","mt_nchar","mt_fasta")
...
write.csv(gtable3,file = "summarize/3ss_hunglun_gtag_only_candidates_100_read_count_30.csv")
...
ag_caused_sig <- unique(c(setdiff(set2p,set4),setdiff(set3p,set5)))  # 34

gtable3$add_ag_cause_significant <- NA
gtable3$add_ag_cause_significant[which(gtable3$mt %in% ag_caused_sig)] <- TRUE   # 34

######### find if there is minus ag cause significant
gtable3$mt[intersect(intersect(unique(c(which(!is.na(gtable3$spliced_unspliced_sig)),which(!is.na(gtable3$canonical_noncan_sig)),which(!is.na(gtable3$canonical_noncan_un_sig)))),which(gtable3$can_is_most_wt_mt==TRUE)),which(gtable3$AG=="reduce_AG"))]  #126
...
m_ag_caused_sig <- unique(c(setdiff(set2p,set6),setdiff(set3p,set7)))  # 136

gtable3$minus_ag_cause_significant <- NA
gtable3$minus_ag_cause_significant[which(gtable3$mt %in% m_ag_caused_sig)] <- TRUE   # 136
