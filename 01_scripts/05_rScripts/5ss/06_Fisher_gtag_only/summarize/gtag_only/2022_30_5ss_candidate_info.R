...
### use hunglun's condition: fdr 0.05, reads 100, |wt-mt| > 30
...
set <- su_f %>% filter(wt_sum_3 == 3) %>% filter(mt_sum_3 == 3) %>% select(mt) %>% unlist() #1756
set1_ <- su_f %>% filter(qval_3 == 3) %>% filter(se_difference_3 == 3) %>% select(mt) %>% unlist() #42
set1 <- intersect(set1_,set) #26
set2_ <- cn_f %>% filter(qval_3 == 3) %>% filter(se_difference_3 == 3) %>% select(mt) %>% unlist() #286
set2 <- intersect(set2_,set) #116
set3_ <- cnu_f %>% filter(qval_3 == 3) %>% filter(se_difference_3 == 3) %>% select(mt) %>% unlist() #345
set3 <- intersect(set3_,set) #165
...
# mark significant candidates: 557
...
# mark significant candidates by hunglun's criteria: 551
gtable5$spliced_unspliced_fdr0.05_reads100_30 <- NA
gtable5$spliced_unspliced_fdr0.05_reads100_30[which(gtable5$mt %in% set1)] <- "su"
gtable5$canonical_noncan_fdr0.05_reads100_30 <- NA
gtable5$canonical_noncan_fdr0.05_reads100_30[which(gtable5$mt %in% set2)] <- "cn"
gtable5$canonical_noncan_un_fdr0.05_reads100_30 <- NA
gtable5$canonical_noncan_un_fdr0.05_reads100_30[which(gtable5$mt %in% set3)] <- "cnu"
...
most <- na.omit(intersect(intersect(s1$most,s2$most),s3$most))  # 2473
...
colnames(gtable5)[12:15] <- c("wt_can_intron_start","wt_can_intron_end","wt_nchar","wt_fasta")

gtable5 <- merge(gtable5,final5[,c(2,5,6,7,8)],by.x="mt",by.y="name",all.x=TRUE)
colnames(gtable5)[16:19] <- c("mt_can_intron_start","mt_can_intron_end","mt_nchar","mt_fasta")
...
write.csv(gtable5,file = "summarize/gtag_only/5ss_gtag_only_candidates_100_read_count_30.csv")
...
gt_caused_sig <- unique(c(setdiff(set2p,set4),setdiff(set3p,set5)))  # 8
...
gtable5$mt[intersect(intersect(unique(c(which(!is.na(gtable5$spliced_unspliced_sig)),which(!is.na(gtable5$canonical_noncan_sig)),which(!is.na(gtable5$canonical_noncan_un_sig)))),which(gtable5$can_is_most_wt_mt==TRUE)),which(gtable5$GT=="reduce_GT"))]  #112
...
m_gt_caused_sig <- unique(c(setdiff(set2p,set6),setdiff(set3p,set7)))  # 114
