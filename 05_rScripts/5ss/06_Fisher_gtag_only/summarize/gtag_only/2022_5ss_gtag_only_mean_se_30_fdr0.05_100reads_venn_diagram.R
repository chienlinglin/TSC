...
## mean 
wt_s_e <- na.omit(cnu[,c(2,8,10,18,20,28,30,3)])
...
se_difference <- abs(wt_s_e[,c(9)]-mt_s_e[,c(8)])
se_difference_1 <- cbind(se_difference,mt_s_e)
se_difference_1$se_difference[is.na(se_difference_1$se_difference)] <- 0
...
su_f <- merge(su_f,se_difference_1[,c(2,1)],by.x ="mt",by.y="mt")
...
cn_f <- merge(cn_f,se_difference_1[,c(2,1)],by.x ="mt",by.y="mt")
...
cnu_f <- merge(cnu_f,se_difference_1[,c(2,1)],by.x ="mt",by.y="mt")

# Generate 3 sets of 200 words
set <- su_f %>% filter(wt_sum_3 == 3) %>% filter(mt_sum_3 == 3) %>% select(mt) %>% unlist() #1756
set1_ <- su_f %>% filter(qval_3 == 3) %>% filter(se_difference >= 30) %>% select(mt) %>% unlist() #44
set1 <- intersect(set1_,set) #26
set2_ <- cn_f %>% filter(qval_3 == 3) %>% filter(se_difference >= 30) %>% select(mt) %>% unlist() #336
set2 <- intersect(set2_,set) #129
set3_ <- cnu_f %>% filter(qval_3 == 3) %>% filter(se_difference >= 30) %>% select(mt) %>% unlist() #398
set3 <- intersect(set3_,set) #178
...
venn.diagram(
  ...
  filename = '2022_5ss_significant_gtag_only_mean_se_30_fdr0.05_100reads_venn_diagramm.png',
  output=TRUE,
  ...
  cat.pos = c(-16, 1, 160),
  cat.dist = c(0.03, 0.05, -0.02),
  cat.fontfamily = "sans",
  rotation = 1
)

#####
# set1(spliced_unspliced): 41
# set2(can_noncan): 503
# set3(can_noncan+un): 528
### total: 597
### 2644 wt-mt pair
### 597/2644= 22.58%
